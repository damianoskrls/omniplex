...
// ============================================================
router.get('/clients/lookup', requireClientAdmin, async (req, res) => {
  const bizId = req.admin.businessId;
  const { phone } = req.query;
  if (!phone) return res.json({ status: 'not_found' });

  const digits = String(phone).replace(/\D/g, '');
  if (digits.length < 7) return res.json({ status: 'not_found' });
  const last9 = digits.slice(-9);

  try {
    // 1. Is there a global_users record with this phone?
    ...
    // 3. Is this global_user already a client in THIS gym?
    const [localRows] = await db.query(
      `SELECT u.id, u.full_name, u.email, u.phone, u.account_status
       FROM users u
       WHERE u.business_id = ? AND u.global_user_id = ?
         AND (u.deleted_at IS NULL OR u.deleted_at > NOW())`,
      [bizId, gu.id],
    );
    if (localRows.length) {
      return res.json({ status: 'in_this_gym', user: localRows[0] });
    }

    // 4. Do they have a pending/approved join_request for THIS gym?
    ...
    // 5. They exist elsewhere — find which gyms
    const [otherRows] = await db.query(
      `SELECT DISTINCT b.name AS gym_name
       FROM users u
       JOIN businesses b ON b.id = u.business_id
       WHERE u.global_user_id = ? AND u.business_id != ?
         AND (u.deleted_at IS NULL OR u.deleted_at > NOW())
       LIMIT 3`,
      [gu.id, bizId],
    );
...
router.post('/clients/invite', requireClientAdmin, async (req, res) => {
  ...
  try {
    ...
    if (existing.length) {
      await db.query(
        `UPDATE gym_join_requests SET status = 'pending', updated_at = NOW() WHERE id = ?`,
        [existing[0].id],
      );
    } else {
      await db.query(
...
// ============================================================
// POST /api/client-admin/clients/add-global — Add an existing
// global_user directly to this gym (active, no invite flow)
// ============================================================
router.post('/clients/add-global', requireClientAdmin, async (req, res) => {
  ...
  try {
    ...
    const [existing] = await db.query(
      `SELECT id FROM users WHERE business_id = ? AND global_user_id = ?
       AND (deleted_at IS NULL OR deleted_at > NOW())`,
      [bizId, global_user_id],
    );
    if (existing.length) return res.status(409).json({ error: 'Ο πελάτης είναι ήδη εγγεγραμμένος' });
    ...
    await db.query(
      `UPDATE gym_join_requests SET status = 'approved', updated_at = NOW()
